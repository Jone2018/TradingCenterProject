	...
	return (<div className="top-login hidden-xs">
		<div className="container">
		  <div className="clearfix">
			...
			<div className="tl-info fr"> <i className="icon icon-user txt-primary"></i> <span>{nicname}</span> UID：<span>{uid}</span> [<LinkLogout />] </div>
		  </div>
		</div>
	  </div>);
}
...
function MenuItem(props)
{
	...
	return (<li className={props.className}><a href={props.url} className="hvr-underline-from-center" target={target}>{txt}</a></li>);
}

function MenuItemOTC(props)
{
	var txt = props.children || props.text;
	
	return (<li className={props.className}>
			<a href={props.url} className="hvr-underline-from-center dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false">{txt}<span className="caret"></span></a>
			<ul className="dropdown-menu">
            <li><a href="otc-list-buy.html">购买</a></li>
            <li><a href="otc-list-sell.html">出售</a></li>
            <li><a href="otc-ads.html">发布广告</a></li>
          </ul>
		</li>);
}
...
var MTradeCate = React.createClass({
	...
	componentDidMount: function() {
		$.getJSON("/api/market/market_info",{
	...
	return (<div className="visible-xs visible-sm cate-dd">
          <div className="dropdown"> <a href="#" id="dLabel" className="btn-block lbl" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"> <span id="dd-cointype">{currLbl}</span> <span className="icon-angle-down"></span> </a>
...
function NavHeader(props)
{
	...
	else if(dType == "2")
	{
  	...
  	return (
        ...
        <button type="button" className="navbar-toggle collapsed" data-toggle="collapse" data-target="#bs-navmenu" aria-expanded="false"> <span className="icon-bar"></span> <span className="icon-bar"></span> <span className="icon-bar"></span> </button>
		{dd}
        <a className="navbar-brand" href="index.html"><img src="/static/images/bitbiex_logo.png" /></a> </div>
	);
}
...
function NavLeft(props)
{
  	return (
		<ul className="nav navbar-nav">
			...
			<MenuItemOTC className="mitem mm-otc dropdown" url="#otc.html" text={gettext('场外交易')} />
        </ul>
	);
}
